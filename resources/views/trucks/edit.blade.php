  ...
  <div class="bg-white rounded-lg shadow-sm">
    <div class="p-6">
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">Atualize as informações do caminhão conforme necessário.</p>
      </div>

      <form action="{{ route('trucks.update', $truck->id) }}" method="POST" id="truck-form">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <select class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('tipo_combustivel') border-red-500 @enderror" 
              ...
              <option value="Diesel" {{ old('tipo_combustivel', $truck->tipo_combustivel) == 'Diesel' ? 'selected' : '' }}>Diesel</option>
              <option value="Gasolina" {{ old('tipo_combustivel', $truck->tipo_combustivel) == 'Gasolina' ? 'selected' : '' }}>Gasolina</option>
              <option value="Etanol" {{ old('tipo_combustivel', $truck->tipo_combustivel) == 'Etanol' ? 'selected' : '' }}>Etanol</option>
              <option value="Flex" {{ old('tipo_combustivel', $truck->tipo_combustivel) == 'Flex' ? 'selected' : '' }}>Flex</option>
            </select>
          ...
          <div>
            ...
            <input type="number" 
                   ...
                   value="{{ old('carga_suportada', $truck->carga_suportada) }}"
          ...
          <!-- Quilometragem -->
          <div>
            <label for="quilometragem" class="block text-sm font-medium text-gray-700 mb-1">Quilometragem</label>
            <input type="number" 
                   step="0.01" 
                   class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('quilometragem') border-red-500 @enderror" 
                   id="quilometragem" 
                   name="quilometragem" 
                   value="{{ old('quilometragem', $truck->quilometragem) }}"
                   placeholder="Ex: 0.00">
            @error('quilometragem')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          ...
          <div>
            ...
            <select class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('status') border-red-500 @enderror" 
                    id="status" 
                    name="status">
              <option value="1" {{ old('status', $truck->status) ? 'selected' : '' }}>Ativo</option>
              <option value="0" {{ !old('status', $truck->status) ? 'selected' : '' }}>Inativo</option>
            </select>
            @error('status')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          ...
          <!-- Última Revisão -->
          <div>
            <label for="ultima_revisao" class="block text-sm font-medium text-gray-700 mb-1">Última Revisão</label>
            <input type="date" 
                   name="ultima_revisao" 
                   id="ultima_revisao" 
                   value="{{ $truck->ultima_revisao?->format('Y-m-d') }}"
