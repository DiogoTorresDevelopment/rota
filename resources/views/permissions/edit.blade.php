...
<div class="flex flex-col">
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
      ...
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span class="text-gray-500">Editar Grupo</span>
        </div>
      </li>
    </ol>
  </nav>
  ...
  <div class="bg-white rounded-lg shadow-sm">
    <div class="p-6">
      ...
      <form action="{{ route('permissions.update', ['permission' => $group->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <select class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('status') border-red-500 @enderror" 
                    id="status" 
                    name="status">
              <option value="ativo" {{ old('status', $group->status) == 'ativo' ? 'selected' : '' }}>Ativo</option>
              <option value="inativo" {{ old('status', $group->status) == 'inativo' ? 'selected' : '' }}>Inativo</option>
            </select>
            @error('status')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
                ...
                <div class="space-y-3">
                  ...
                  <label class="flex items-center">
                    <input type="checkbox" 
                           ...
                           name="permissions[]" 
                           value="{{ $permission->id }}"
                           {{ in_array($permission->id, $group->permissions->pluck('id')->toArray()) ? 'checked' : '' }}>
